...
class RateLimiter {
     ...
     */
    getSenderKey(senderId, hourWindow) {
     ...
     */
    getNextHourStart(from = new Date()) {
     ...
     */
    getCurrentHourEnd(from = new Date()) {
     ...
     */
    async getCurrentCount(senderId, atTime = new Date()) {
     ...
     */
    async checkAndIncrement(senderId, limit, atTime = new Date()) {
        const key = this.getSenderKey(senderId, atTime);
    ...
    `;
        ...
        const result = await this.redis.eval(script, 1, key, String(limit), String(expireAt));
        return {
            allowed: result[0] === 1,
     ...
     */
    async getRemainingSlots(senderId, limit) {
