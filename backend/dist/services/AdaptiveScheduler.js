...
exports.adaptiveScheduler = exports.AdaptiveScheduler = void 0;
...
const SseEmitter_1 = require("./SseEmitter");
/**
...
class AdaptiveScheduler {
    constructor() {
        this.campaignRepo = database_1.AppDataSource.getRepository(Campaign_1.Campaign);
        this.jobRepo = database_1.AppDataSource.getRepository(EmailJob_1.EmailJob);
        this.senderRepo = database_1.AppDataSource.getRepository(Sender_1.Sender);
    }
    /**
    ...
    async previewPlan(params) {
        ...
        const effectiveLimit = Math.min(params.hourlyLimit, sender?.hourlyLimit ?? parseInt(process.env.MAX_EMAILS_PER_HOUR_PER_SENDER || '100'));
        const slotsRemaining = await RateLimiter_1.rateLimiter.getRemainingSlots(params.senderId, effectiveLimit);
        const estimatedTimes = await RateLimiter_1.rateLimiter.computeEstimatedSendTimes({
        ...
        const hoursSpanned = Math.ceil((lastTime.getTime() - firstTime.getTime()) / (1000 * 60 * 60)) + 1;
        return {
            totalEmails: params.totalEmails,
            estimatedTimes,
            hoursSpanned,
    ...
    async schedule(params) {
        ...
        SseEmitter_1.sseEmitter.emit('campaign:created', {
            campaignId: campaign.id,
            totalRecipients: params.recipients.length,
            firstSendTime: estimatedTimes[0],
            lastSendTime: estimatedTimes[estimatedTimes.length - 1],
        });
        console.log(`✅ Campaign ${campaign.id} scheduled: ${params.recipients.length} emails, ` +
...
exports.adaptiveScheduler = new AdaptiveScheduler();
